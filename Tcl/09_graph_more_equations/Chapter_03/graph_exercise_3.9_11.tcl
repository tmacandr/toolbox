#!/usr/bin/wish
# ************************************************************************
# FILE: graph_exercise_3.9_11.tcl
#
# DESCRIPTION:
...
for { set x $min_x } { $x <= $max_x } { set x [expr $x + 0.0001] } {
   ...
   set inv_x [expr ( 1.0 / $x2 ) ]

   set diff_x [ expr ( $x2 - $inv_x ) ]

   set diff_x2 [ expr ($diff_x * $diff_x) ]

   set y [expr ( 1.0 / $diff_x2 ) ]

   set x1 [expr ($horiz_xi + ((($x - $min_x) / $rel_domain_dist) * $abs_domain_dist))]
   set y1 [expr ($vert_yi + (((($rel_range_dist / 1) - $y) / $rel_range_dist)  * $abs_range_dist))]
