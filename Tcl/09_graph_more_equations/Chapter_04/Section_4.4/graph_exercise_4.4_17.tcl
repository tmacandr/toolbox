#!/usr/bin/wish
# ************************************************************************
# FILE: graph_exercise_4.4_17.tcl
#
# DESCRIPTION:
#    Tcl script to graph the equation from Exercise 4.4-17
#*************************************************************************
...
proc Draw_Y_Grid_Marks { } {
   ...
   for { set y $min_y } { $y <= $max_y } { set y [expr $y + 5] } {

       set x_up [expr ($vert_xi + 5)]
       set x_dwn [expr ($vert_xi - 5)]
...
for { set x $min_x } { $x <= $max_x } { set x [expr $x + 0.001] } {
   ...
   if { $x < 0 } {
       ...
       set x13 [ ::tcl::mathfunc::pow $x13 0.33 ]

       set x13 [expr (-$x13)]

   } else {

       set x13 [ ::tcl::mathfunc::pow $x 0.33 ]

   }

   set x43 [ expr ( $x13 * $x13 * $x13 * $x13) ]

   set y [expr ( 8 * $x13 + $x43 ) ]
