#!/usr/bin/wish
# ************************************************************************
# FILE: graph_exercise_4.2_21.tcl
#
# DESCRIPTION:
...
for { set x $min_x } { $x <= $max_x } { set x [expr $x + 0.001] } {
   ...
   #
   #               3      2
   # Draw y = 1/3 x  + 2 x  + 4 x + 1
   #
   set x2 [ expr ( $x * $x ) ]

   set x3 [ expr ( $x2 * $x ) ]

   set y [ expr ( 0.3 * $x3 + 2 * $x2 + 4 * $x + 1 ) ]

   set x1 [expr ($horiz_xi + ((($x - $min_x) / $rel_domain_dist) * $abs_domain_dist))]
   set y1 [expr ($vert_yi + (((($rel_range_dist / 2) - $y) / $rel_range_dist)  * $abs_range_dist))]

   set magenta_equation [.the_canvas create rectangle $x1 $y1 $x1 $y1 -width 0 -fill magenta ]
...
for { set x $min_x } { $x <= $max_x } { set x [expr $x + 0.001] } {
   ...
   #
   #               3        2
   # Draw y = 1/3 x  - 5/2 x  + 4 x + 1
   #
   set x2 [ expr ( $x * $x ) ]

   set x3 [ expr ( $x2 * $x ) ]

   set y [ expr ( 0.3 * $x3 - 2.5 * $x2 + 4 * $x + 1 ) ]
