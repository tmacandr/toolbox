#!/usr/bin/wish
# ************************************************************************
# FILE: graph_exercise_4.1_17.tcl
#
# DESCRIPTION:
...
for { set x $min_x } { $x <= $max_x } { set x [expr $x + 0.0001] } {
   ...
   set radicand [ expr ( 2 * $x - 5 ) ]

   if { $radicand < 0.0 } {

      set temp [expr ( - $radicand )]

      set x3root [ ::tcl::mathfunc::pow $temp 0.3 ]

      set $x3root [expr ( - $x3root )]

   } else {

      set x3root [ ::tcl::mathfunc::pow $radicand 0.3 ]

   }

   set y [expr ($x2 * $x3root)]
